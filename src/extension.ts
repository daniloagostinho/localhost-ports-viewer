...
async function refreshDockerPortMap(): Promise<void> {
  ...
  if (Date.now() < dockerUnavailableUntil) { return; }
  try {
...
// ─── Process map (full names via ps-list) ────────────────────────────────────

interface ProcMapEntry { name: string; cmd: string }
 ...
 */
let processMap: Map<string, ProcMapEntry> = new Map();

async function refreshProcessMap(): Promise<void> {
  processMap = new Map();
  try {
...
async function identifyService(processName: string, pid: string, port: string, isWindows: boolean): Promise<ServiceInfo> {
  ...
  let result: ServiceInfo = { platform: resolvedName };
...
// ─── Concurrency-limited candidate resolution ────────────────────────────────

interface PortCandidate { port: string; pid: string; procName: string; isWindows: boolean }
...
class LocalhostPortsWebviewProvider implements vscode.WebviewViewProvider {
  ...
  async resolveWebviewView(webviewView: vscode.WebviewView) {
    ...
    if (webviewView.visible) { this.startAutoRefresh(); }
  ...
  private startAutoRefresh() {
    this.stopAutoRefresh();
    const interval = Math.max(1000, getConfig().get<number>('refreshInterval', 5000));
    this._timer = setInterval(() => {
      if (this._view?.visible) { this.refreshPorts(true); }
    }, interval);
  }
  ...
  private async refreshPorts(silent = false) {
