...
function debugLog(msg, ...args) {
    if (getConfig().get('debugLogs', false)) {
        console.log(`[LocalhostPortsViewer] ${msg}`, ...args);
    }
}
// ─── execWithTimeout ──────────────────────────────────────────────────────────
async function execWithTimeout(cmd, timeoutMs) {
    const ms = timeoutMs ?? getConfig().get('commandTimeout', 5000);
    const timeout = new Promise((_, reject) => setTimeout(() => reject(new Error(`Command timed out after ${ms}ms: ${cmd}`)), ms));
...
async function collectPortsFallback() {
    debugLog('Using fallback port detection (common ports only)');
    const results = [];
    await Promise.all(COMMON_PORTS.map(async ({ port, info }) => {
        try {
